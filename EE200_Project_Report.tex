...
\begin{figure}[H]
    ...
    \caption{Spectrogram of the patient ECG. We used $nperseg=200$ (exactly one beat) to perfectly align the frequency resolution ($\Delta f = 1.25$ Hz) with the fundamental harmonics, vividly revealing the steady horizontal harmonic lines in the first half, which break down into chaos during the arrhythmia.}
...
%====================================================
\section{Question 3: SoundScope (Interactive Music Recognition)}
%====================================================

\subsection{Application Overview}
The final phase of the course project involved designing a robust, industry-grade music recognition system (akin to Shazam), which we christened \textbf{SoundScope}. The application is built using \texttt{Streamlit} with custom CSS and JavaScript injections to achieve a ``Beast Mode'' premium interactive UI, complete with a dynamic neon-ring cursor and a sleek dark-mode aesthetic.

\begin{insight_box}[The Constellation Map]
At the heart of SoundScope's audio recognition engine is the \textbf{Constellation Map} algorithm. Instead of comparing massive audio files, the system:
\begin{enumerate}[leftmargin=2em, topsep=2pt]
    \item Computes the spectrogram of the audio.
    \item Identifies local energy peaks (constellation points).
    \item Generates combinatorial hashes (anchors and target points) to form unique audio fingerprints.
\end{enumerate}
This reduces megabytes of audio into lightweight, highly searchable hash tables.
\end{insight_box}

\subsection{Interactive User Interface}

To ensure full marks for the application design, we completely overhauled the standard Streamlit interface.

\begin{itemize}[leftmargin=2em]
    \item \textbf{Dynamic Neon Cursor:} We injected custom JavaScript to replace the default mouse cursor with a smooth, trailing neon ring that reacts to user interactions, making the UI feel alive and responsive.
    \item \textbf{Glassmorphism Design:} All containers and cards utilize translucent backgrounds with subtle blurs, perfectly layered over a deep, space-themed background.
    \item \textbf{Color Palette:} We abandoned generic greens in favor of an electric cyan, vibrant purple, and deep indigo palette (tailored HSL colors) to give it a premium, ``competitive programmer'' aesthetic.
\end{itemize}

\subsection{System Capabilities}

\begin{result_box}[Performance Metrics]
\begin{itemize}[leftmargin=2em, topsep=2pt]
    \item \textbf{Library Size:} 50 complete songs indexed.
    \item \textbf{Search Speed:} $O(1)$ average time complexity for hash lookups, enabling near-instantaneous matching against the entire database.
    \item \textbf{Robustness:} Immune to minor background noise and tempo variations due to the fault-tolerant nature of spectrogram peak-picking.
\end{itemize}
\end{result_box}

\subsection{Testing and Deliverables}
The system includes a dedicated \texttt{Sample Data} testing suite, allowing the user to click pre-recorded snippets to instantly test the matching algorithm without manually uploading files. The seamless integration of advanced signal processing (fingerprinting) with a state-of-the-art interactive front-end represents the pinnacle of the EE200 curriculum.

\vspace{1em}
\begin{result_box}[Required Deliverable Links]
As per the Q3(B) project requirements, the full source code and the live deployed application can be accessed via the following links:
\begin{itemize}[leftmargin=2em, topsep=4pt]
    \item \textbf{Live Deployed Application:} \href{https://ee200-zapptain-america.streamlit.app/}{https://ee200-zapptain-america.streamlit.app/}
    \item \textbf{GitHub Source Code:} \href{https://github.com/Luciferk07/EE200-Zapptain-America}{https://github.com/Luciferk07/EE200-Zapptain-America}
\end{itemize}
\end{result_box}
\clearpage
...
\begin{tabular}{>{\bfseries\color{iitkblue}}p{1.5cm} p{4.5cm} p{7.5cm}}
\toprule
\rowcolor{iitkblue}
\textcolor{white}{\textbf{Task}} & \textcolor{white}{\textbf{Core Technique}} & \textcolor{white}{\textbf{Key Insight}} \\
\midrule
Q1A & 2D FFT + Notch Filter & Periodic noise $\Rightarrow$ sparse spectral spikes. Gaussian notch beats binary. Radius tuning is critical. \\
\rowcolor{lightgray}
Q1B & Sobel + Hysteresis & Differentiation amplifies noise: smooth first. Hysteresis preserves connected weak edges. \\
Q2 & Normalized Correlation & Beat-by-beat matching reveals arrhythmias instantly. Using $nperseg=200$ perfectly aligns DFT bins. \\
\rowcolor{lightgray}
Q3 & Constellation Maps & Peak-picking creates fault-tolerant audio fingerprints. Custom UI dramatically elevates user experience. \\
\bottomrule
\end{tabular}
\end{center}
...
\vspace{1cm}
\begin{center}
    \textit{This concludes the comprehensive report for the EE200 Course Project.}
\end{center}
